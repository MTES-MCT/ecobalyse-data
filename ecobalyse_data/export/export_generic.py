...
def compute_processes_generic(
    ...
    """Compute ProcessGeneric dicts with metadata enrichment (forest management).
    ...
    """
    # Use unfiltered full file (contains object/veli processes that are
    # filtered out of the regular processes_impacts.json)
    full_path = processes_impacts_path.replace(
        "processes_impacts.json", "processes_impacts_full.json"
    )
    if os.path.exists(full_path):
        processes_impacts_path = full_path
...
def activities_to_processes_generic_json(
    ...
    generic_dicts = compute_processes_generic(
        activities, processes_impacts_path, cpu_count
    )
    ...
    aggregated = remove_detailed_impacts(generic_dicts)
    for path in aggregated_output_paths:
        export_json(aggregated, path)
        logger.info(
            f"Exported {len(aggregated)} aggregated generic processes to {path}"
        )
